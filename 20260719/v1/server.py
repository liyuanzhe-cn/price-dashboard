...
ATH_TICKERS = list(WATCHLIST.values()) + list(ETF_WATCHLIST.values()) + list(INDICES.values()) + list(METALS.values()) + [MSTR_TICKER, ASST_TICKER]
...
def fetch_dividends(ticker_map):
    ...
    dividendYield×100(已经转成百分比数字,不是0.0282这种小数)。
    ...
    for name, sym in ticker_map.items():
        ...
        try:
            ...
            # 少数情况下雅虎财经dividendYield字段本身就已经是百分比数值(比如4.2表示4.2%)
            # 而不是小数(0.042),两种格式混用过,这里用一个粗略阈值判断:正常股息率
            # 很少会超过30%,超过这个数就认为它本身已经是百分比,不再×100
            yield_pct = None
            if yld is not None:
                yield_pct = yld if yld > 30 else yld * 100
