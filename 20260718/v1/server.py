...
# ------------------------------------------------------------------
# 方法论/数据来源说明日志——原来这些技术性备注是直接显示在页面上的.sub小字里,
# v21按你的要求挪到这个单独的可下载txt里,页面本身只保留干净的数字,不起眼的
# "下载说明日志"按钮放在页面最底部。
# ------------------------------------------------------------------
METHODOLOGY_LOG_TEXT = """行情看板 - 数据来源与方法论说明
================================
这份文件汇总了看板页面上原本以小字形式展示的技术性备注,方便你需要的时候查阅,
不影响正常看板阅读。

【链上指标 - 抓取频率】
链上指标统一按"每日更新"档处理:第一次抓到之前立刻抓,之后每天美东时间07:XX
(XX是启动时随机出来的分钟数)才会重新抓一次,不会每60秒请求。

【链上指标 - 数据来源】
CVDD/NUPL/RHODL/盈利占比/60日涨幅这几项数值来自bgeometrics.com。
200周均线/200日均线改用OKX公开行情接口自己算K线均值。
恐惧贪婪指数来自alternative.me。
减半倒计时的当前区块高度来自mempool.space。
MVRV换成looknode-proxy的mCapRealizedRatio(取时间戳最新一条)。
MVRV-Z换成独立的btc-cache接口。
Realized Price不再单独抓取,改成用"现价÷MVRV"反推。

【MSTR/ASST面板】
以上BTC持仓/市值/mNAV等字段抓取自bitcointreasuries.net,不是自己算的,更新频率
见后端配置(默认20分钟);现价单独走雅虎财经,60秒左右刷新。
标黄并带"≈"的数字是网站原文抓不到时,用已抓到的其他字段换算出来的近似值,不是
网站原始数字。

【MSTR/BTC涨跌幅对比基准】
基准:MSTR 2020-08-11开盘价$14.20(拆股复权后,和现价同一口径,不需要额外换算);
BTC同日开盘价$11,899.87。这两个是写死的历史事实,不是每次重新抓取的数据。

【Beta系数/市值(雅虎财经)】
这两个字段走雅虎财经较"重"的接口,放在每日更新档,不是每60秒刷新。

【STRC飞轮数据】
这几项抓取自mstr.fuckbtc.com,它本身是从strategy.com官网和SEC 8-K文件整理的
二手聚合页(strategy.com自己的页面是纯客户端渲染抓不到的),不是自己算的。

【BTC市值/市占率/TOTAL2/TOTAL3】
你给的bitbo.io和TradingView这两个链接实测都是纯客户端渲染,抓不到具体数值。改用
CoinGecko的/global接口(免费公开,和现价用的同一个数据源)算出来的:总市值×BTC
占比=BTC市值;总市值-BTC市值=TOTAL2;再减去ETH市值=TOTAL3。"不含稳定币"的市占率
额外用了CoinGecko的稳定币分类市值做分母调整。这几项5分钟刷新一次(比价格慢,
因为是偏宏观的慢变量)。链接指向的还是bitbo.io/TradingView原网站方便你对照查看,
但页面上显示的数字是我们自己算出来的,不是从那两个网站抓的。

【贵金属】
黄金稳定币(XAUT)走CoinGecko;周大福首饰金是零售报价,不是交易所行情,单独一行
展示,更新频率约3小时一次;锂没有统一的全球现货代码,用LIT ETF代理。

【其他加密货币】
"距历史高点"直接用CoinGecko自带字段,不是自己算的;OKX是交易所名字不是代币,
这里按平台币OKB处理。

【股市风险指标评级体系】
下面6个(除席勒市盈率外)都来自currentmarketvaluation.com,用的是同一套评级体系:
按当前值偏离历史均值的标准差数量分5档——
  强烈低估(<-2个标准差,约2%概率)
  低估(-2~-1,约14%)
  合理(-1~+1,约70%)
  高估(+1~+2,约14%)
  强烈高估(>+2,约2%)
席勒市盈率(in2013dollars.com)单独一套3档:≤25低风险 / 25~35中风险 / >35高风险,
不是同一体系,不要混用比较。
之前尝试过在卡片里嵌入官方图表,后来删掉了(外链图片显示不出来,大概率是被防盗链
拦截,与其挂个裂图不如不放)。桌面端宽屏幕会自动排成两栏,手机端自动变成一栏。
席勒市盈率数据源是in2013dollars.com(之前用过的multpl.com一直抓不到数据,已换掉)。

【9个链上指标的三色分类阈值】(红=过热/高估,黄=中性,绿=熊市/低估)
这些具体分界线是你确认过的经验区间,不是某个单一权威来源认证过的科学标准,
不同分析师的具体数字会有出入,仅供参考:
  MVRV: <0.9低估 · 0.9~1.8中立 · >1.8高估
  MVRV-Z: <0极度低估 · 0~5中立 · >5高估
  Realized Price: 参考MVRV=0.9/1.8对应的价位区间
  Balanced Price: 跌破代表深熊绝对底部
  CVDD: 现价在其上方/下方的百分比,CVDD是历史有效的铁底
  200周均线: 现价/200周均线,<1倍熊市 · 1~3倍中性 · >3倍过热
  60日累计涨幅: <-20%安全区 · -20%~50%中立 · 50%~100%风险 · >100%极高风险
  SOPR: SOPR=1是链上持币者卖出时的盈亏分界线,<0.95恐慌 · 0.95~1.05中立 · >1.05高估
  Puell Multiple: <0.5矿工投降的底部区间 · 0.5~2.5中立 · >2.5高估
  NUPL: <0恐慌/底部 · 0~0.25怀疑 · 0.25~0.5乐观/中立 · 0.5~0.75信念/高估 · >0.75极度贪婪/顶部
  RHODL比率: 接近绿色通道(<350)底部低估 · 接近红色通道(>50000)顶部高估
  盈利中的百分比: <50底部极度悲观 · 50~85中立 · >85阶段性/周期性顶部
    ...
    def do_GET(self):
        ...
        if path == "/us.html":
            try:
                ...
                self._send_json({"error": "us.html not found next to server.py"}, 404)
            return
        if path == "/us500_data.json":
            # 这个文件是独立的us500_fetch.py脚本(通过cron定时)生成的,server.py这里
            # 只是原样读文件返回,不会触发抓取,不会增加主循环负担。
            try:
                with open("us500_data.json", "rb") as f:
                    body = f.read()
                self.send_response(200)
                self.send_header("Content-Type", "application/json; charset=utf-8")
                self.send_header("Access-Control-Allow-Origin", "*")
                ...
                self._send_json({"error": "us500_data.json还没生成,先手动跑一次 python3 us500_fetch.py --limit 20 测试"}, 404)
            return
