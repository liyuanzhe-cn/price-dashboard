...
CACHE_SCHEMA_VERSION = 8  # v17加入looknode-proxy覆盖SOPR/Puell/Balanced Price,数据结构可能变了
...
MSTR_ANCHOR_PRICE_PRE_SPLIT = 124.70   # 拆股前价格,现价需要×10才能跟它比
BTC_ANCHOR_PRICE = 11410.53            # 同一天BTC收盘价
...
def compute_since_mstr_adoption(mstr_price_now, btc_price_now):
    ...
    try:
        if mstr_price_now is not None:
            mstr_pre_split_equiv = mstr_price_now * 10
            out["mstr_return_pct"] = (mstr_pre_split_equiv / MSTR_ANCHOR_PRICE_PRE_SPLIT - 1) * 100
