...
WATCHLIST = {
    ...
    # TODO: "21Shares"本身是瑞士的私人公司(21.co旗下,发行加密货币ETP的机构),
    # 没有一个统一的"21Shares股票"可以买卖,不像MSTR/Metaplanet是真实上市公司。
    # 我没有替你猜一个具体的ETP代码(比如ARKB之类),因为选错了意义完全不同,
    # 这里先留空,你确认具体想要哪个标的(某只ETP?还是别的东西)后我再补上。
    "21Shares(待确认具体标的)": None,
}
...
def fetch_ath_drawdown(tickers):
    ...
    用月线而不是日线是为了减少一次性下载的数据量/内存占用。
    逐个ticker顺序处理、处理完立刻丢弃DataFrame。"""
    out = {}
    for sym in tickers:
        try:
