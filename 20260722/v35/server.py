    ...
    try:
        ...
        with STATE_LOCK:
            ...
            if data.get("onchain"):
                ...
                # 修正上一小版把PSIP 1.18误当成“盈利/亏损”而算反的缓存，无需清空整份缓存。
                supply = STATE["onchain"].get("supply_in_profit_pct")
                if isinstance(supply, dict) and supply.get("profit_loss_ratio") is not None:
                    ratio = float(supply["profit_loss_ratio"])
                    profit_pct = 100 / (1 + ratio)
                    supply.update({
                        "value": profit_pct,
                        "loss_pct": 100 - profit_pct,
                        "loss_profit_ratio": ratio,
                    })
                    supply.pop("profit_loss_ratio", None)
...
def apply_btc_cache_signals(out, btc_cache):
    ...
    try:
        ...
        if math.isfinite(psip_ratio) and psip_ratio >= 0:
            # 接口PSIP口径是“亏损供应 ÷ 盈利供应”。例如1.18代表约54%亏损、46%盈利。
            profit_pct = 100 / (1 + psip_ratio)
            out["supply_in_profit_pct"] = {
                "value": profit_pct,
                "loss_pct": 100 - profit_pct,
                "loss_profit_ratio": psip_ratio,
                "unit": "percentage",
...
def build_onchain_explanations(out, live_btc_price):
    ...
    # 盈利供应占比：btc-cache的原始PSIP是“亏损供应/盈利供应”的比值，
    ...
    if supply_val is not None:
        ...
        balance_text = f"盈利约{supply_val:.1f}% / 亏损约{loss_val:.1f}%"
        if supply_val <= 45:
...
# ------------------------------------------------------------------
# 官方指标：财政部国债 + FRED收录的BLS/BEA/DOL官方序列
# ------------------------------------------------------------------

def _fetch_fred_series(series_id, limit=15):
    """读取FRED免Key CSV，返回按日期升序的有效数值。"""
    response = requests.get(
        "https://fred.stlouisfed.org/graph/fredgraph.csv",
        params={"id": series_id}, headers=HEADERS, timeout=20,
    )
    ...
    for row in csv.DictReader(io.StringIO(response.text)):
        ...
        if raw in (None, "", "."):
            continue
        try:
            rows.append((row.get("observation_date"), float(raw)))
        except (TypeError, ValueError):
            continue
    if len(rows) < 2:
        raise ValueError(f"FRED {series_id}有效数据不足")
    return rows[-limit:]


def _macro_yoy_card(series_id, name, source, unit="%"):
    rows = _fetch_fred_series(series_id, 15)
    if len(rows) < 14:
        raise ValueError(f"FRED {series_id}不足14个月，无法计算同比")
    latest_date, latest_value = rows[-1]
    previous_date, previous_value = rows[-2]
    actual = (latest_value / rows[-13][1] - 1) * 100
    previous = (previous_value / rows[-14][1] - 1) * 100
    return {
        "name": name, "actual": round(actual, 2), "expected": None,
        "previous": round(previous, 2), "unit": unit, "as_of": latest_date,
        "source": source, "expected_note": "官方机构不发布市场预期",
    }
...
def fetch_official_indicators():
    ...
    definitions = {
        "cpi": ("CPIAUCSL", "CPI消费者价格指数（同比）", "BLS via FRED"),
        "ppi": ("PPIFIS", "PPI最终需求价格指数（同比）", "BLS via FRED"),
        "pce": ("PCEPI", "PCE个人消费支出价格指数（同比）", "BEA via FRED"),
    }
    for key, (series_id, name, source) in definitions.items():
        try:
            card = _macro_yoy_card(series_id, name, source)
            card["url"] = f"https://fred.stlouisfed.org/series/{series_id}"
            out["items"][key] = card
            _clear_errors(f"official:{key}:")
        except Exception as error:
            _record_error(f"official:{key}", error)

    # 非农：PAYEMS单位为千人，展示最新一个月相对上月的变化。
    try:
        payrolls = _fetch_fred_series("PAYEMS", 3)
        out["items"]["nonfarm"] = {
            "name": "非农就业人数变化", "actual": round(payrolls[-1][1] - payrolls[-2][1]),
            "expected": None,
            "previous": round(payrolls[-2][1] - payrolls[-3][1]) if len(payrolls) >= 3 else None,
            "unit": "K", "as_of": payrolls[-1][0], "source": "BLS via FRED",
            "url": "https://fred.stlouisfed.org/series/PAYEMS",
            "expected_note": "官方机构不发布市场预期",
        }
        _clear_errors("official:nonfarm:")
    except Exception as error:
        _record_error("official:nonfarm", error)

    # 初领失业金：ICSA为季调后的每周申请人数。
    try:
        claims = _fetch_fred_series("ICSA", 2)
        out["items"]["jobless_claims"] = {
            "name": "初领失业金人数", "actual": claims[-1][1], "expected": None,
            "previous": claims[-2][1], "unit": "人", "as_of": claims[-1][0],
            "source": "U.S. Department of Labor via FRED",
            "url": "https://fred.stlouisfed.org/series/ICSA",
            "expected_note": "官方机构不发布市场预期",
        }
        _clear_errors("official:jobless_claims:")
    except Exception as error:
        _record_error("official:jobless_claims", error)
