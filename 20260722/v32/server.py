...
def fetch_recent_youtube_videos():
    ...
    for author, channel_url in YOUTUBE_CHANNELS:
        try:
            ...
            # 重新解析一次；500属于YouTube临时故障，不反复请求频道页面。
            if r.status_code == 404:
                channel_ids.pop(channel_url, None)
                refreshed_id = _resolve_youtube_channel_id(channel_url)
                if refreshed_id and refreshed_id != channel_id:
                    channel_id = refreshed_id
                    channel_ids[channel_url] = channel_id
                    r = requests.get(feed_url, params={"channel_id": channel_id}, headers=HEADERS, timeout=15)
            r.raise_for_status()
