...
def fetch_fundamentals(ticker, display_ticker=None):
    ...
    out = {
        ...
        "revenue": None,
        "earnings_date": None, "earnings_date_end": None,
        "earnings_timestamp": None, "earnings_date_is_estimate": None,
    }
    try:
        ...
        # 复用本来就要请求的.info，不为1500只股票额外调用calendar/earnings_dates。
        # Yahoo有时给精确时间earningsTimestamp，有时只给预计时间窗Start/End。
        earnings_exact = info.get("earningsTimestamp")
        earnings_start = info.get("earningsTimestampStart")
        earnings_end = info.get("earningsTimestampEnd")
        earnings_ts = earnings_exact or earnings_start
        try:
            if earnings_ts is not None:
                earnings_ts = int(earnings_ts)
                out["earnings_timestamp"] = earnings_ts
                out["earnings_date"] = datetime.utcfromtimestamp(earnings_ts).strftime("%Y-%m-%d")
                out["earnings_date_is_estimate"] = earnings_exact is None or (
                    earnings_start is not None and earnings_end is not None and earnings_start != earnings_end
                )
            if earnings_end is not None:
                out["earnings_date_end"] = datetime.utcfromtimestamp(int(earnings_end)).strftime("%Y-%m-%d")
