...
def apply_btc_cache_signals(out, btc_cache):
    ...
    try:
        ...
        pass
    try:
        psip = float(btc_cache["psip"])
        if math.isfinite(psip) and 0 <= psip <= 100:
            out["supply_in_profit_pct"] = {
                "value": psip, "date": btc_cache.get("psip_date"),
...
def build_onchain_explanations(out, live_btc_price):
    ...
    # 盈利供应占比：用户给出的历次周期底部约为34%~45%。<=45%视作进入历史底部
    # 参考区，否则只提示尚未进入；这是历史参照，不等于买卖建议。
    ...
    if supply_val is not None:
        ...
        # 当前Worker偶尔返回约1.x；它既不像百分数，也不能乘100（会超过100%）。
        # 仍展示原始值方便排查，但不把它误判成“1%盈利供应/历史大底”。
        if supply_val <= 2:
            exp["supply_in_profit_pct"] = {"zone": "yellow", "text": f"上游返回原始值{supply_val:.3f}，与0~100%的历史口径不一致，暂不作底部判断 · {history}{date_text}"}
        elif supply_val <= 45:
            exp["supply_in_profit_pct"] = {"zone": "green", "text": f"已进入底部参考区(≤45%) · {history}{date_text}"}
        else:
            exp["supply_in_profit_pct"] = {"zone": "red", "text": f"尚未进入底部参考区(>45%) · {history}{date_text}"}
    else:
