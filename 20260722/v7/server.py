...
ETF_WATCHLIST = {
    ...
    "三倍做空纳指(SQQQ)": "SQQQ",         # ProShares三倍做空纳指ETF
    ...
    "三倍做多纳指(TQQQ)": "TQQQ",            # ProShares三倍做多纳指100 ETF
    "七巨头ETF(MAGS)": "MAGS",               # Roundhill Magnificent Seven ETF
    "两倍做多标普500(SSO)": "SSO",           # ProShares两倍做多标普500 ETF
    "两倍做多标普500(UPRO)": "UPRO",         # ProShares三倍做多标普500 ETF(注:UPRO实际是三倍,不是两倍,你原话写的"两倍",这里按你给的名字保留,如果要精确名字需要告诉我改成"三倍做多标普500(UPRO)")
}
...
# 已停用：交易所里同名币/股票代币可能与美股代码碰撞（例如CVX），会产生数量级错价。
# 512MB VPS上采用Yahoo证券行情更稳妥；休市后显示最新正式/盘前盘后报价，不伪装成24/7股价。
ENABLE_BITGET_STOCK_OVERLAY = False
